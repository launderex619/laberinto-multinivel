 ...
 */
public class ResolverAStarThread extends AsyncTask<Integer, HeuristicPathTree.Node, HeuristicPathTree> {
     ...
     */
    public ResolverAStarThread(HeuristicPathTree.Node[][] nodes,
    ...
    protected HeuristicPathTree doInBackground(Integer... values) {
        ...
        expandedNodes.add(nodes[initialY][initialX]);
        ...
        setFitnessFirst(nodes[initialY][initialX]);
        //nodes[initialY][initialX].setStep(actualStep++);
    ...
    protected void onPostExecute(HeuristicPathTree heuristicPathTree) {
        ...
        heuristicPathTree.drawDotTree(HeuristicPathTree.A_STAR);
    ...
    /**
     * Methon to set the h(n) and g(n) values to the first node of the tree.
     * @param Node Anchor node.
     */
    private void setFitnessFirst(HeuristicPathTree.Node Node) {
        float distance = 0f;
        switch (measureMode) {
            case EUCLIDIANA:
                //distance = (float) Math.sqrt(Math.pow(lastX - Node.getPosX(), 2) + Math.pow(lastY - Node.getPosY(), 2));
                distance = (float) Math.sqrt( (Math.pow(lastX - Node.getPosX(), 2))  +  (Math.pow(lastY - Node.getPosY(), 2)) );
    ...
    private void setFitness(HeuristicPathTree.Node response, HeuristicPathTree.Node Father) {
        ...
        switch (measureMode) {
            case EUCLIDIANA:
                ...
                response.setAccumulative( Father.getAccumulative() + response.getCost() );
